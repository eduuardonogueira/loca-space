...
export function ReservationDetailsModal({ isOpen, onClose, reservation, onCancelClick }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 font-sans leading-none">
      <div className="bg-white rounded-[2rem] w-full max-w-xl overflow-hidden relative shadow-2xl animate-in zoom-in-95 duration-200">
        <button onClick={onClose} className="absolute top-5 right-5 z-30 bg-white/90 p-1.5 rounded-full shadow-sm hover:bg-white text-gray-500">
          <X size={20} />
        </button>
        ...
        <div className="relative h-60 w-full flex gap-1 p-1 bg-gray-50">
          <div className="relative flex-[2] rounded-l-2xl overflow-hidden">
            <Image src={images[currentImg]} alt="Principal" fill className="object-cover" />
          </div>
          <div className="relative flex-1 rounded-r-2xl overflow-hidden cursor-pointer group" onClick={() => setCurrentImg((currentImg + 1) % images.length)}>
            <Image src={images[(currentImg + 1) % images.length]} alt="Próxima" fill className="object-cover opacity-60 transition-opacity group-hover:opacity-80" />
        ...
        <div className="p-10 text-left">
          <h2 className="text-xl font-bold text-gray-900 mb-1 leading-tight">{reservation.title}</h2>
          ...
          <div className="flex gap-10 mb-8 text-sm font-semibold text-gray-700">
            <div className="flex items-center gap-2"><Calendar size={18} className="text-gray-900" /> {reservation.date}</div>
            <div className="flex items-center gap-2 text-gray-400 font-bold"><Clock size={18} /> {reservation.startTime} - {reservation.endTime}</div>
          </div>
          ...
          <div className="mb-8">
            <p className="text-3xl font-[900] text-gray-900 tracking-tight">R$ 1.000</p>
          </div>
